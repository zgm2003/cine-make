...
export function composeProductionBrief(contract) {
  return [
    ...
    `Mode: ${contract.mode === 'visual' ? 'image-output mode; image generation may run after the text plan is stable.' : 'draft mode; do not generate images yet.'}`,
    '',
...
export function composeImagegenBrief(contract) {
  ...
  return [
    ...
    '',
    `Mode: ${contract.mode === 'visual' ? 'image-output mode' : 'draft mode'}. In draft mode, prepare prompts only and do not spend time generating images.`,
    '',
    '## What to generate',
    '',
