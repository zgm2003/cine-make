...
test('skill keeps user prompts natural and internal output rules implicit', async () => {
  ...
  assert.match(skill, /逐条视频文本/)
  assert.match(skill, /每 5 条|Every 5/u)
})
...
test('skill disables Canvas package output and routes to ChatGPT feed', async () => {
  ...
  assert.match(skill, /Canvas package output is disabled|Canvas commands are deprecated and disabled/u)
  ...
  assert.match(skill, /Original-fidelity rules|原著守则/u)
  assert.match(skill, /Shot-language rules|镜头语言规则/u)
  assert.match(skill, /Xiaoyunque camera tags|小云雀运镜标签库/u)
  assert.match(skill, /镜头前推｜强调情绪靠近/u)
  assert.match(skill, /穿越机运镜｜高速掠过空间/u)
  assert.match(skill, /Art Direction|ART_DIRECTION/)
