...
test('skill keeps user prompts natural and internal output rules implicit', async () => {
  ...
  assert.match(skill, /canvas-project\.zip/)
...
test('skill routes manual Canvas work to canvas-pack instead of image generation', async () => {
  ...
  assert.match(skill, /Do not expose `draft` \/ `visual` as user modes/i)
  assert.match(skill, /Do not create `deliverable\.md`, `storyboard-images\/`/i)
