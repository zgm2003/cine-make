...
test('替嫁国漫 Seedance feed uses the proven single-line shot-text format', () => {
  ...
  assert.match(markdown, /^1 日 内 许家大厅 许怡宁 .*三尺青锋.*极特写.*我宁愿死，也不嫁给那个哑巴废物/mu)
  assert.match(markdown, /^3 日 内 许家大厅 许正言 .*三尺青锋.*扫向角落里的许悠然.*嫡女拒婚，全家竟逼庶女替嫁/mu)
  assert.match(markdown, /^5 日 内 许家大厅暗角 江凡 .*江凡始终平静.*眼底闪过一丝冷光.*这场戏，该我收场了/mu)
  assert.match(markdown, /用户实际只用两块/u)
  assert.match(markdown, /## 每5条复制制作块/u)
  assert.match(markdown, /### 第1组｜第1-5条/u)
  assert.match(markdown, /甩摇（快速横移带过）/u)
  assert.match(markdown, /焦点转移（推镜到眼神定格）/u)
  ...
  assert.match(markdown, /上传参考图：许家大厅 = 图片1（场景参考）；许怡宁 = 图片2（人物参考）；许悠然 = 图片3（人物参考）；江凡 = 图片4（人物参考）；许正言 = 图片5（人物参考）；三尺青锋 = 图片6（道具参考）/u)
  assert.doesNotMatch(markdown, /## 参考资产绑定|## 全局负面约束|## 原著守则|## 镜头语言规则|## 小云雀运镜标签库|## 逐条视频文本|## 底部备注栏可复制/u)
  assert.doesNotMatch(markdown, /^1 .*\n\n2 /mu)
  assert.doesNotMatch(markdown, /主要场景|含泪三视图|中景 \+ 平视或轻微低机位|心理悬疑|暴风雨|真人电影质感/u)
})
