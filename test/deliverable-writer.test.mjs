...
test('draft mode exposes only deliverable.md and storyboard-images to users', async () => {
  ...
  try {
    ...
    assert.match(deliverable, /# 1\. 项目理解/)
    assert.match(deliverable, /# 2\. 全局视觉风格/)
    assert.match(deliverable, /# 3\. 资产规划/)
    assert.match(deliverable, /# 4\. 导演视角拆剧/)
    assert.match(deliverable, /# 5\. 正式分镜输出/)
    assert.match(deliverable, /# 6\. 一致性提醒/)
    assert.match(deliverable, /【分镜1】[\s\S]*镜头分析：/u)
    assert.match(deliverable, /【分镜1】[\s\S]*即梦静帧提示词：/u)
    assert.match(deliverable, /【分镜1】[\s\S]*即梦视频提示词：/u)
