...
test('visual mode produces the AI-short-drama image package contract', async () => {
  ...
  try {
    ...
    assert.doesNotMatch(deliverable, /二级动画/)
    assert.doesNotMatch(deliverable, /焦点按主体、关键物、异常信号顺序收束/)
...
test('isolated mansion visual deliverable exposes four cinematic character reference prompts', async () => {
  ...
  try {
    ...
    assert.match(deliverable, /满版构图/)
    assert.match(deliverable, /不留顶部空白/)
    assert.match(deliverable, /只显示角色名称和身高/)
    assert.doesNotMatch(deliverable, /年龄：30岁左右|年龄：32岁左右|年龄：45岁左右|年龄：24岁左右/)
    assert.doesNotMatch(deliverable, /预留干净信息栏/)
    assert.doesNotMatch(deliverable, /预留.*信息栏/)
