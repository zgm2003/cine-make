...
test('抖音仙界第1章 feed stays in chapter 1 and does not preload later chapter assets', () => {
  ...
  assert.match(markdown, /鬼王宗宗门大殿 = 图片1（场景参考）/u)
  assert.match(markdown, /林夜 = 图片2（人物参考）/u)
  assert.match(markdown, /神级文娱系统界面 = 图片3（界面参考）/u)
  assert.match(markdown, /天机一型手机 = 图片4（道具参考）/u)
...
test('抖音仙界第2章 feed covers phone validation and recruiting without chapter 3 design props', () => {
  ...
  assert.match(markdown, /姹女教山门 = 图片3（场景参考）/u)
  const baiQingxuan = pack.assets.find((asset) => asset.title === '白清玄')
  assert.ok(baiQingxuan)
  assert.match(baiQingxuan.prompt, /沿用第 1 章生成图/u)
  assert.match(baiQingxuan.prompt, /白清玄黑纱造型/u)
  assert.match(baiQingxuan.prompt, /不重新生成/u)
  assert.match(baiQingxuan.prompt, /同一张脸、同一发型、同一体型、眉心黑莲印记/u)
  assert.match(markdown, /### 图片6 = 白清玄/u)
  assert.match(markdown, /上传第 1 章已生成的「白清玄黑纱造型」作为本章图片6/u)
  assert.match(markdown, /白清玄 = 图片6（人物参考）/u)
  assert.match(markdown, /幽冥虎王 = 图片7（坐骑妖兽参考）/u)
...
test('抖音仙界第3章 feed starts at training and ends on costume design hook', () => {
  ...
  assert.match(markdown, /林夜音色=林夜\.mp3/u)
  assert.match(markdown, /白清玄音色=白清玄\.mp3/u)
  assert.match(markdown, /姹女教宗门大殿 = 图片1（场景参考）/u)
  assert.match(markdown, /### 图片3 = 白清玄/u)
  assert.match(markdown, /上传第 1 章已生成的「白清玄黑纱造型」作为本章图片3/u)
  assert.match(markdown, /白清玄 = 图片3（人物参考）/u)
  assert.match(markdown, /姹女教女修群像 = 图片4（群像参考）/u)
  assert.match(markdown, /抖音教学玉简 = 图片6（道具参考）/u)
