...
test('cli writes a Seedance all-reference feed without storyboard artifacts', async () => {
  ...
  try {
    ...
    assert.match(feed, /GPT-image-2 参考图生成提示词/u)
    assert.match(feed, /## 每5条复制制作块/u)
    assert.doesNotMatch(feed, /## 小云雀运镜标签库|## 逐条视频文本|## 底部备注栏可复制/u)
    assert.match(feed, /最左侧单独的上半身\+头部细节展示/u)
    assert.match(feed, /三视图为一张图/u)
    assert.match(feed, /16:9/u)
    assert.match(feed, /### 图片1 = 雪山之巅/u)
    assert.match(feed, /### 图片2 = 老年道清三视图/u)
    assert.match(feed, /### 图片3 = 麒麟幼兽三视图/u)
    assert.match(feed, /上传参考图：雪山之巅 = 图片1（场景参考）；老年道清三视图 = 图片2（人物参考）；麒麟幼兽三视图 = 图片3（异兽参考）/u)
    assert.doesNotMatch(feed, /上传参考图：图片1｜雪山之巅/u)
