...
- A good prop reference image prompt exists only after the prop-value gate passes. Do not create prompts for low-content props just because they appear in source text. For approved props, ask for one isolated object, like a single decisive weapon or story-critical artifact. Do not ask for surrounding people, hands, tables, rooms, multiple variants, or bundled props.
